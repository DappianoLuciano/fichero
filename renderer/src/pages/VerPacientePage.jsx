...
export default function VerPacientePage({ paciente, onVolver, onPacienteActualizado }) {
  ...
  const [modalStep,       setModalStep]       = useState("camera"); // "camera" | "preview"
  const [fotosCapturadas, setFotosCapturadas] = useState([]); // [{foto, observaciones}]
  ...
  // refs para usar en el keydown sin stale closures
  ...
  async function cargarRecetas() {
    const data = await window.api.listarRecetas(paciente.id);
    setRecetas(data);
  }
  ...
  async function guardarNombre() {
    ...
    try {
      const updated = await window.api.actualizarPaciente({ id: paciente.id, nombre: n });
  ...
  // Versión para teclado (usa refs para evitar closures stale)
  ...
  async function guardarReceta() {
    ...
    setGuardando(true);
    try {
      await window.api.crearReceta({ pacienteId: paciente.id, fechaOrden, fotos: todasFotos });
  ...
  async function eliminarReceta(id) {
    await window.api.eliminarReceta(id);
  ...
  async function guardarEdicion() {
    ...
    try {
      await window.api.actualizarReceta({ id: editandoReceta.id, fechaOrden: editFecha, fotos: editFotos });
  ...
  const nFotos     = fotosCapturadas.length + 1; // fotos que se guardarán
      ...
      {modalOpen && (
        <div className="modalOverlay" onClick={cerrarModal}>
          <div className="modalCard" style={{ width: "min(680px, calc(100vw - 36px))" }} onClick={(e) => e.stopPropagation()}>
            ...
            {/* Miniaturas de fotos ya confirmadas */}
            {fotosCapturadas.length > 0 && (
